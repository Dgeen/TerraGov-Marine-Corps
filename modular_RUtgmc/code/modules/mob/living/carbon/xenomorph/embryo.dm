...
/mob/living/carbon/xenomorph/larva/initiate_burst(mob/living/carbon/victim)
	...
	if(CHECK_BITFIELD(victim.restrained_flags, RESTRAINED_XENO_NEST))
		if(victim.job == null)
			SSpoints.add_strategic_psy_points(hivenumber, 10)
			SSpoints.add_tactical_psy_points(hivenumber, 2)
		else if(victim.job.type == /datum/job/survivor/rambo)
			SSpoints.add_strategic_psy_points(hivenumber, 50)
			SSpoints.add_tactical_psy_points(hivenumber, 10)
		else
			SSpoints.add_strategic_psy_points(hivenumber, 200)
			SSpoints.add_tactical_psy_points(hivenumber, 50)
